...
func printHelp() {
	...
	fmt.Printf("  %s%s\n", flagCol.Render("--new-only"), desc.Render("Skip folders that already exist in the output directory"))
	...
	fmt.Println()
	fmt.Printf("  %s\n", dim.Render("# Only process folders that don't already exist in the output"))
